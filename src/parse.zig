...
pub const Parser = struct {
    ...
    fn assignment(self: *Self, first_variable: ?PossibleLValueExpression) Error!*Node {
        ...
        if (is_local) {
            ...
            while (true) {
                ...
                if (try self.testcharnext(',')) {
                    std.debug.print("Processing variable after comma\n", .{});
                    variable = try self.primaryexp();
                } else {
                    break;
                }
            }
            std.debug.print("Checking for '='\n", .{});
            try self.checkcharnext('=');
            _ = try self.explist1(&values);
        }
    ...
    fn exprstat(self: *Self) Error!*Node {
        ...
        if (expression.node.id == .call) {
            ...
            // stat -> assignment
            std.debug.print("Calling assignment\n", .{});
    ...
    fn prefixexp(self: *Self) Error!PossibleLValueExpression {
        std.debug.print("prefixexp: token={s}({any}) char={?}\n", .{self.state.token.nameForDisplay(), self.state.token.id, self.state.token.char});
        switch (self.state.token.id) {
            .name => {
                ...
                try self.nextToken();
                return PossibleLValueExpression{ .node = &node.base, .can_be_assigned_to = true };
            },
            .single_char => switch (self.state.token.char.?) {
                '(' => {
